...
public class JobData extends JFrame implements ActionListener,MouseListener {
	...
	public JobData() {
		...
		btnOmit.setBounds(501, 325, 67, 32);
	...
	public boolean fullFields() {
		if(txtNumExterior.getText().length() > 0 && txtDesc.getText().length() > 0 && txtDom.getText().length() > 0 && txtTelefono.getText().length() > 0) {
			return true;
		}
		else{
	...
	public void addDatosEmpleo(String name,String ap1,String ap2) {
		try {
			c.update("INSERT INTO clientes_empleo (id_cliente,Descripcion,Domicilio,Num_Exterior,Num_Interior,Telefono) VALUES ("+getIdClienteByName(name,ap1,ap2)+",'"+txtDesc.getText()+"','"+txtDom.getText()+"','"+txtNumExterior.getText()+"','"+txtNumInter.getText()+"','"+txtTelefono.getText()+"');");
